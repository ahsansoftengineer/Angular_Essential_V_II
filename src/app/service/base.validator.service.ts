...
import { Validation, ValidationMessage } from '../interface/validators/validations';
...
export class ValidatorService {
  ...
  _error_server(httpErrorResponse: HttpErrorResponse) {
    let server_response = httpErrorResponse.error;
  ...
  */
  _val(fn: Partial<string> = '', param: Partial<ValidatorParam> = {
    ...
    return (
      ...
      if (
        ...
        return  MSG('HYPHEN','Hyphen not allowed at start and end')
  ...
  // This doesnt Work Well for FormArray
  // Now not in Use
      ...
      Object.keys(group.controls).forEach((key: string) => {
        ...
        if (acc instanceof FormGroup ||
          ...
          if (
            acc.status == 'INVALID' ||
            acc.status == 'PENDING' ||
            !Custom.emptyCheck(acc.value)// async Validators
          ) {
            console.error(this._error_control(acc));
            if(!Custom.emptyCheck(acc.value))console.log(key);
